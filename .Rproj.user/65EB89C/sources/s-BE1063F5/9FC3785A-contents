...
fit_likelihood <- function(data, node_list, grid_A, cutoffs_A, cutoffs_J, type_A = c("above", "below", "equal"), type_J = c("above", "below", "equal"), lrnr =  Lrnr_glmnet$new(), lrnr_A = lrnr, lrnr_C = lrnr, lrnr_N = lrnr, lrnr_J = lrnr, verbose = FALSE ) {
  ...
  for(cutoff_J in cutoffs_J) {
    ...
    print(length(task_J$folds[[1]][[2]]) + length(task_J$folds[[1]][[3]]))

    lrnr_J_trained <- lrnr_J$train(task_J)
  ...
  lrnr_A_RCDF <- Lrnr_density_discretize$new(Lrnr_pooled_hazards$new(lrnr_A), n_bins = length(grid_A), breaks = grid_A, predict_type = type)
  ...
  likelihood <- list(J = lrnrs_J, A = lrnr_A_trained, N = lrnr_N_trained, C = lrnr_C_trained, grid_A = grid_A, cutoffs_A = cutoffs_A_1, cutoffs_J = cutoffs_J, type_A = type_A, folds_n = folds_n)
  return(likelihood)
}
...
get_likelihoods <- function(fits, data, node_list, unexpanded_likelihood = NULL) {
  ...
  print("Computing J")
  for(cutoff_J in cutoffs_J) {
    task_J <- make_task_J(data, node_list, cutoff_J, train = F, folds = folds_nt)
    print(dim(task_J$X))
  ...
  if(is.null(unexpanded_likelihood)){
    ...
    for(cutoff_A in grid_A) {
      task_A <- make_task_A(data, node_list, cutoff = cutoff_A, train = F, folds = folds_nt)
